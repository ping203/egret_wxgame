...
class Utils{
     ...
     * 绘制rect
     */
    public static createRect(x:number,y:number,width:number,height:number,radius:number,bgColor:number,alpha:number):eui.Rect{
        let rect:eui.Rect=new eui.Rect(width,height,bgColor);
        rect.x=x;
        rect.y=y;
        rect.alpha=alpha;
